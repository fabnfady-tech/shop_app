...
* `shop_app` - project identifier (lowercase, no spaces, i.e. "snake_case") used for bundle IDs and package names.
* `` - project description.
* `shop_app` - executable or bundle name on disk.
* `Shop App` - display name shown in window titles and about app dialogs.
* `com.example` - org name in reverse domain name notation, e.g. `com.mycompany.myproject`.
